...
const CustomPrevArrow: React.FC<HomeProps & { onClick: React.MouseEventHandler<HTMLButtonElement> }> = ({ theme, onClick }) => (
    <button onClick={onClick} className={`absolute top-[50%] -left-10 flex items-center justify-center w-[37px] h-[49px] slick-arrow custom-prev-arrow border border-[#2C9CF0] text-[#2C9CF0] hover:bg-[#2C9CF0] hover:text-white`}>
        {`<`}
    </button>
);

const CustomNextArrow: React.FC<HomeProps & { onClick: React.MouseEventHandler<HTMLButtonElement> }> = ({ theme, onClick }) => (
    <button onClick={onClick} className={`absolute top-[50%] -right-10 flex items-center justify-center w-[37px] h-[49px] slick-arrow custom-next-arrow border border-[#2C9CF0] text-[#2C9CF0] hover:bg-[#2C9CF0] hover:text-white`}>
        {`>`}
    </button>
);
...
export default function Home() {
    ...
    const settings = {
        ...
        slidesToShow: 3,
        slidesToScroll: 1,
        prevArrow: <CustomPrevArrow theme={theme} onClick={() => sliderRef.current?.slickPrev()} />,
        nextArrow: <CustomNextArrow theme={theme} onClick={() => sliderRef.current?.slickNext()} />
    };
    ...
    return (
        <>
            <button onClick={HandleThemeChange} className={`fixed top-5 md:top-10 right-5 md:right-8 w-8 h-8 md:w-10 md:h-10`}>
            ...
            <div className={`p-5 md:py-10 md:px-20 w-full h-full flex flex-col items-center ${theme === 'light' ? 'bg-[#F7F7F8]' : 'bg-[#292B32]'}`}>
                ...
                <div className={`mt-[50px] w-full flex flex-col items-center p-10 bg-gradient-to-b ${theme === 'light' ? 'text-black from-[#F9F8FF] to-[#F3F9FF]' : 'text-[#ffffff] from-[#18282A] to-[#221A2C]'}`}>
                    ...
                    <div className={`mt-[30px] w-[90%] md:w-[78%]`}>
                        <Slider ref={sliderRef} {...settings}>
                            {ticketData.map((ticket, index)=>
                                <Ticket key={index} ticketData={ticket} theme={theme} />
                            )}
                        </Slider>
                    </div>
                </div>
